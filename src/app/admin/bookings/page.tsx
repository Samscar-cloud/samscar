...
const COLUMNS: { id: Status; label: string; color: string; bg: string; border: string; glow: string; icon: string }[] = [
  {
    id: 'PENDING',
    label: 'En attente',
...
function BookingCard({
  ...
  return (
    ...
    >
      {/* Status dot */}
      <div className={`absolute top-3 right-3 w-2 h-2 rounded-full ${col.color.replace('text-', 'bg-')} shadow-sm`} />
      ...
      <div className="flex flex-col gap-2">
        ...
        {/* Service */}
        <div className={`text-xs font-medium px-2 py-1 rounded-md w-fit ${col.bg} ${col.color}`}>
...
function SortableBookingCard({ booking, onDelete }: { booking: Booking; onDelete: (id: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <BookingCard booking={booking} onDelete={onDelete} isDragging={isDragging} />
    </div>
  )
}
...
function KanbanColumn({
  ...
  return (
    ...
    >
      {/* Column header */}
      <div className={`flex items-center justify-between px-4 py-3 rounded-t-2xl border-b ${col.border}`}>
        <div className="flex items-center gap-2">
      ...
      <div ref={setNodeRef} className="flex flex-col gap-3 p-3 flex-1">
        ...
        {bookings.length === 0 && (
          <div className={`flex-1 flex items-center justify-center rounded-xl border-2 border-dashed transition-colors duration-200
            ${isOver ? `${col.border} ${col.bg}` : 'border-white/10'} min-h-[120px]`}>
            <p className={`text-xs transition-colors ${isOver ? col.color : 'text-gray-600'}`}>
              {isOver ? '⬇️ Déposer ici' : 'Glisser ici'}
            </p>
          </div>
        )}
      </div>
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (targetColumn) {
      ...
      if (booking && booking.status !== targetColumn.id) {
        // Optimistic update
        setBookings(prev => prev.map(b => b.id === draggedId ? { ...b, status: targetColumn.id } : b))
    ...
    if (targetBooking) {
      ...
      if (draggedBooking && draggedBooking.status !== targetBooking.status) {
        // Optimistic update
        setBookings(prev => prev.map(b => b.id === draggedId ? { ...b, status: targetBooking.status } : b))
        await updateStatus(draggedId, targetBooking.status)
      }
    }
  }

  const byStatus = (status: Status) => bookings.filter(b => b.status === status)
    ...
    <div className="min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            <span className="text-primary-400 font-semibold">{total}</span> réservations au total — glissez pour changer le statut
          </p>
        </div>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-6 text-xs text-gray-500">
        <span className="flex items-center gap-1.5">
          <span className="text-base">✋</span> Glisser une carte pour changer le statut
        </span>
        <span className="flex items-center gap-1.5">
          <span className="text-base">📧</span> Email automatique envoyé au client à chaque changement
        </span>
        <span className="flex items-center gap-1.5">
